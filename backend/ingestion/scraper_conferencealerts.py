...
Large academic + professional conference directory.
Plain HTML, very scraper-friendly.
"""
import asyncio
import httpx
    ...
    async def fetch(self) -> List[EventCreate]:
        ...
        async with httpx.AsyncClient(headers=HEADERS, follow_redirects=True, timeout=15) as client:
            for category, url in CATEGORY_URLS:
                ...
                # rows are typically <tr> inside a results table
                rows = soup.select("table tr, .conference-list li, .conf-item")
                if not rows:
                    rows = soup.select("div.event")
