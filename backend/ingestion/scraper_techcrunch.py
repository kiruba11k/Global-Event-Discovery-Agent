...
def _parse_tc_date(text: str) -> str:
    ...
    if m2: return f"{m2.group(3)}-{MONTHS.get(m2.group(1)[:3],'01')}-{m2.group(2).zfill(2)}"
    return ""
    ...
    async def fetch(self) -> List[EventCreate]:
        ...
        for card in cards[:20]:
            try:
                ...
                start_date = _parse_tc_date(date_text)
                if not start_date:
                ...
                events.append(EventCreate(
                    id=self.make_id(), source_platform="TechCrunch",
                    ...
                    start_date=start_date, end_date=start_date, city=city, country="USA",
                    category="tech", industry_tags="startup,venture capital,tech,AI,product",
